...
class QuotesController < ApplicationController
  def index
    ...
    #                  .select(
    #                    :quotes_add, :quotes_edit, :quotes_delete, :quotes_email
    #                  ).first
  ...
  def gridajaxdata
    ...
    resultdata.data.each do |resultline|
      ...
      classification = contract_types.find { |x| x[0] == resultline[index] }[1] if resultline[index] != -1
      if classification
        resultline[index] = classification
      else
        resultline[index] = ""
      end
      ...
      analysis = analysis_list.find { |x| x[0] == resultline[index] }[1] if resultline[index] != -1
      if analysis
        resultline[index] = analysis
      else
        resultline[index] = ""
      end
      ...
      project_manager = project_managers.find { |x| x[0] == resultline[index] }[1] if resultline[index] != -1
      if project_manager
        resultline[index] = project_manager
      else
        resultline[index] = ""
      end
  ...
  def new
    ...
    #@access_rights = UserAccessRight.where(user_id: session[:user_id])
    #                  .select(
    #                    :quotes_edit, :quotes_order, :quotes_invoice, :quotes_email
    #                  ).first
