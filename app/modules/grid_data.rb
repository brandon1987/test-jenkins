  ...
  def getGridData(databaseModel, gridparams, fieldsarray, stridcolumn, strjoins, strwhere, strorder, additional_fields_array, group="")
    ...
    records.each do |record|
      ...
      record.attributes.each_with_index do |(fieldname,fieldvalue),index|
          ...
          rowdata.merge!({"#{index}"=>escape_nil(fieldvalue)}) if fieldname!="_id" && !additional_fields_array.include?(fieldname)
          customattributes.merge!({"#{fieldname}"=>escape_nil(fieldvalue)}) if fieldname!="_id" && additional_fields_array.include?(fieldname)
