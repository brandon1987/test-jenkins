...
module AmazonStorage
	...
	#Return an array of the file in this cloud folder
	def getAmazonFileList(folder)
		filearray  = Array.new
		s3         = getS3()
		bucket     = s3.buckets["storage.constructionmanager.net"]
		foldername = "#{Rails.env}#{session[:company_id]}/#{folder}"
	...
	class S3File
	  def initialize(filename, last_modified, file_size)
	    @filename     = filename
	    @lastmodified = datetime_format(last_modified)
	    @filesize     = Filesize.from("#{file_size} B").pretty
	  end
	end
